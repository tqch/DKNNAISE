...
class AISE:
    ...
    def __init__(self, x_orig, y_orig, hidden_layer=None, model=None, input_shape=None, device=torch.device("cuda"),
                 ...
                 mut_range=(.05, .1), mut_prob=(.05, .1), mut_mode="crossover",
                 decay=(.9, .9), n_population=1000, memory_threshold=.025, plasma_threshold=.005,
    ...
    def generate_b_cells(self, ant, ant_tran, nbc_ind, y_ant=None):
        ...
        if self.keep_memory:
            mem_bcs = torch.stack(mem_bcs).view((-1, 20) + self.input_shape).numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fitness-function", help="Specify a function used to calculate fitness score",
                        default="negative l2")
    ...
    parser.add_argument("--attack-intensity", help="The radius of lp ball constraint on attacks (0-255)", type=int,
                        default=40)
    ...
    parser.add_argument("-n", "--normalize", help="Whether to normalize the flattened vector or not",
                        action="store_true")
    ...
    parser.add_argument("-t", "--adaptive-temp", help="Whether to use heuristic sampling temperature or not",
                        action="store_true")
    ...
    for HIDDEN_LAYER in range(4):
        ...
        aise = AISE(x_train, y_train, model=net, n_neighbors=N_NEIGH, hidden_layer=HIDDEN_LAYER, max_generation=MAX_GEN,
                    normalize=NORMALIZE,
                    avg_channel=AVG_CHANNEL, fitness_function=FITNESS_FUNC, sampling_temperature=SAMPL_TEMP,
        ...
        if NO_KNN:
            ...
            if os.path.exists(
                    "adaptive_cache/knn_proba_{}_{}_{}_{}{}{}.pkl".format(N_TRAIN, N_EVAL, LAYER_NAME, DATA_TYPE_SHORT,
                                                                          "_" + str(EPS) if ATTACK else "",
                                                                          "_n" if NORMALIZE else "")):
                with open("adaptive_cache/knn_proba_{}_{}_{}_{}{}{}.pkl".format(N_TRAIN, N_EVAL, LAYER_NAME,
                                                                                DATA_TYPE_SHORT,
                                                                                "_" + str(EPS) if ATTACK else "",
                                                                                "_n" if NORMALIZE else ""), "rb") as f:
                ...
                knn_proba = knn.predict_proba(ant_conv.flatten(start_dim=1).numpy()
                                              if HIDDEN_LAYER is not None else x_ant.flatten(
                    start_dim=1).detach().cpu().numpy())
                with open("adaptive_cache/knn_proba_{}_{}_{}_{}{}{}.pkl".format(N_TRAIN, N_EVAL, LAYER_NAME,
                                                                                DATA_TYPE_SHORT,
                                                                                "_" + str(EPS) if ATTACK else "",
                                                                                "_n" if NORMALIZE else ""), "wb") as f:
        ...
        if SAVE_RESULT:
            ...
            with open("adaptive_results/result_mnist_{}_{}_{}_{}{}_{}.pkl".format(DATA_TYPE_SHORT, LAYER_NAME, N_TRAIN,
                                                                                  N_EVAL,
                                                                                  "_" + str(EPS) if ATTACK else "",
                                                                                  timestamp), "wb") as f:
                pickle.dump([aise_proba, knn_proba, ant_logs], f)
            with open("adaptive_results/bcells_mnist_{}_{}_{}_{}{}_{}.pkl".format(DATA_TYPE_SHORT, LAYER_NAME, N_TRAIN,
                                                                                  N_EVAL,
                                                                                  "_" + str(EPS) if ATTACK else "",
                                                                                  timestamp), "wb") as f:
