...
class AISE:
    ...
    def generate_b_cells(self, ant, ant_tran, nbc_ind, y_ant=None):
        ...
        for n in range(ant.size(0)):
            ...
            if self.keep_memory:
                mem_bcs.append(
                    curr_gen[fitness_rank[-(self.n_memory + self.n_plasma) + 19:-self.n_plasma:20]].detach().cpu())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval-size", help="Evaluation size", type=int, default=1000)
    ...
    if ATTACK:
        if USE_CACHE:
            if os.path.exists("adaptive_cache/x_mnist_{}_{}v2_{}.pkl".format(DATA_TYPE_SHORT, N_EVAL, EPS)):
                ...
                with open("adaptive_cache/x_mnist_{}_{}v2_{}.pkl".format(DATA_TYPE_SHORT, N_EVAL, EPS), "wb") as f:
    ...
    for HIDDEN_LAYER in range(4):
        ...
        _, _, _, pla_labs, ant_logs = aise(x_ant, y_eval)
        end_time = time.time()
        ...
        if SAVE_RESULT:
            ...
            with open("adaptive_results/result_mnist_{}_{}_{}_{}v2nh{}_{}.pkl".format(DATA_TYPE_SHORT, LAYER_NAME, N_TRAIN,
                                                                                  N_EVAL,
