...
class AISE:
    ...
    '''
    def __init__(self,X_orig,y_orig,X_hidden=[],layer_dims=None,model=None,input_shape=None,device=torch.device("cuda"),
                 n_class=10,n_neighbors=10,query_class="l2",norm_order=2,fitness_function=recip_l2_dist,
                 sampling_temperature=.3,max_generation=20,requires_init=False,mut_range=(.15,.3),mut_prob=(.05,.15),
                 mut_mode="guided",combine_prob=0,decay=None,n_population=1000,memory_threshold=.25,plasma_threshold=.05):
            ...
            for i in range(self.max_generation):
                ...
                if self.decay:
                    ...
                    if pop_fitness < best_pop_fitness:
                        if decay_coef[0] >= math.log(self.mut_range[0]/self.mut_range[1],self.decay[0]) and \
                                decay_coef[1] >= math.log(self.mut_prob[0] / self.mut_prob[1], self.decay[1]): # early stop
                            break
